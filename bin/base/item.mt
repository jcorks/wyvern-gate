...
@:ItemDesign = import(module:'base/item/design.mt');
@:Material = import(module:'base/item/material.mt');
@:ApparelMaterial = import(module:'base/item/apparelmaterial.mt');
...
Item.database.newEntry(
  data : {
    ...
    events : {
      onCreate::(item) {
        @:Edible = import(:'base/item/edible.mt');
...
Item.database.newEntry(data : {
  ...
  events : {
    onCreate ::(item, creationHint) {
      @:Material = import(:'base/item/material.mt');
...
@:Item = databaseItemMutatorClass.createLight(
  ...
  statics : {
    ...
    Improvement : {
      get ::<- import(:'base/item/improvement.mt')
    }
  },
  ...
  interface : {
    defaultLoad::(base, creationHint, qualityHint, materialHint, apparelHint, rngEnchantHint, colorHint, designHint, artsHint, forceEnchant, forceEnchantCount, forceNeedsAppraisal, forceSlotCount) {
      ...
      if (forceSlotCount != empty || base.hasTraits(:TRAIT.HAS_INLET_SLOTS)) ::<= {
        ...
        if (slotCount > 0) ::<= {
          ...
          state.inletSlotData = import(:'base/item/inletset.mt').new(size:slotCount);          
    ...
    setInletSlots::(count) {
      _.state.inletSlotData = import(:'base/item/inletset.mt').new(size:count);  
    },
    ...
    appraise ::{
      ...
      @:item = Item.new(
        base,
        forceEnchantCount: random.integer(from:5, to:7),
        forceEnchant : true,
        rngEnchantHint : true,
    ...
    describe ::(by) {
      ...
      if ((state.base.traits & Item.TRAIT.STRANGE_TO_EQUIP) == 0) 
        windowEvent.queueReader(
          ...
          ,
          lines : 
            state.stats.descriptionRateLinesBase(:this.equipModBase)
