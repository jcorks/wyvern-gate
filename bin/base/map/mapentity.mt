      ...
      if (::? {
        foreach(mapEntity.controller.map.getItemsWithinRadius(
          ...
          if (item.data->type == Location.type && (
            item.data.base.hasAnyTrait(:
              Location.TRAIT.ENTRANCE_HINT |
              Location.TRAIT.EXIT_HINT
            )
          ))
            send(message:true);
...
@:stepEmitter = import(:'core/graphics/particle.mt').new(
  directionMin: 0,
  directionMax: 0,
  directionDeltaMin: 0,
...
@:damageEmitter = import(:'core/graphics/particle.mt').new(
  directionMin: 0,
  directionMax: 360,
  directionDeltaMin: 3,
  directionDeltaMax: 10,
