...
@:addEntity ::(map, landmark, island, state) {
    ...
    @ents = [landmark.island.newInhabitant()]
    ...
    interface : {
        ...
        defaultLoad ::{
            ...
            state.maxEncounters = if (state.isBusy) 
                ...
                (if (_.landmark.floor == 0) 
                    0 
                else 
                    (2+(_.landmark.floor/3)->round)
                )
            ;
        },
        
        step::{
            @:entities = _.landmark.mapEntityController.mapEntities->filter(by::(value) <- value.tag == 'dungeonencounter');
            @:state = _.state;
            ...
            if (!world.battle.isActive &&
                ...
                _.landmark.base.peaceful == false && 
                    (   
                        state.isBusy 
                            || 
                        (Number.random() < 0.1 / (state.encountersOnFloor*(10 / (_.island.tier+1))+1))
                    )
                ) ::<= {
